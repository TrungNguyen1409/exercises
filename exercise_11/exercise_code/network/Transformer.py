...
class ScaledDotAttention(nn.Module):
    ...
    def forward(self,
        ...
        scores = (q @ torch.transpose(k, -2, -1)) / torch.sqrt(torch.tensor(self.d_k, dtype=torch.float32))
        if mask is not None:
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self,
        ...
        q = self.weights_q(q).reshape(batch_size, sequence_length_queries, self.n_heads, self.d_k)
        k = self.weights_k(k).reshape(batch_size, sequence_length_keys, self.n_heads, self.d_k)
        v = self.weights_v(v).reshape(batch_size, sequence_length_keys, self.n_heads, self.d_v)

        q = torch.transpose(q, 1, 2).reshape(batch_size, self.n_heads, sequence_length_queries, self.d_k)
        k = torch.transpose(k, 1, 2).reshape(batch_size, self.n_heads, sequence_length_keys, self.d_k)
        v = torch.transpose(v, 1, 2).reshape(batch_size, self.n_heads, sequence_length_keys, self.d_v)
        # print(mask.size())
        if mask is not None:
        ...
        # print(outputs.size())
        outputs = outputs.view(batch_size, self.n_heads, sequence_length_queries, self.d_v)
...
class EncoderBlock(nn.Module):
    ...
    def forward(self,
        ...
        att_out = self.multi_head(inputs, inputs, inputs, pad_mask)
        att_out = self.layer_norm1(inputs + att_out)
        ffn_out = self.ffn(att_out)
        outputs = self.layer_norm2(att_out + ffn_out)
...
class DecoderBlock(nn.Module):
    ...
    def forward(self,
        ...
        out_att = self.causal_multi_head(inputs,inputs,inputs,causal_mask)
        out_att = self.layer_norm1(inputs + out_att)
        out_cross_att = self.cross_multi_head(inputs,context,context,pad_mask)
        out_cross_att = self.layer_norm2(out_att + out_cross_att)
        out_ffn = self.ffn(out_cross_att)
        outputs = self.layer_norm3(out_cross_att + out_ffn)
